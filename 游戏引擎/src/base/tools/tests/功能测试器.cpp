	...
	const uint64_t& block_size, const coord_double& root, uint64_t test_times)
{
	...
	//坐标查询结果存储
	vector<tree_block_data<Test_Data>*> tree_handle_group{};
	//坐标查询结果存储
	vector<tree_block_data<Test_Data>*> tree_range_group{};
	//当前测试半径存储
	...
	for (; ;)
	{
		...
		for (int test_time = 0; test_time < test_times; test_time++)
		{
			...
			//初始化空指针
			tree_handle_group.push_back(nullptr);
			//查询获取信息
			tree->block_seek(tree_handle_group.back(), coord_group.back());
			//若查询失败
			if (tree_handle_group.back() == nullptr)
			{
				//格式化坐标存储
			...
			{
				//设置数值以待校验
				Test_Data::set_num(*tree_handle_group.back(), gen(-coord_radius_start, coord_radius_start));
				//继续循环
				continue;
			}
		}
		...
		for (int test_time = 0; test_time < test_times; test_time++)
		{
			...
			//若两次查询结果都为空句柄
			if (examiner == nullptr || tree_handle_group[test_time] == nullptr)
			{
				//格式化坐标存储
			...
			//进行查询结果检验
				...
				Test_Data::compare(*tree_handle_group[test_time], *examiner);
		}

		//重置存储器并且释放内存
		coord_group.clear();
		tree_handle_group.clear();
	...
	const uint64_t& block_size, const coord_double& root, uint64_t test_times)
{
	...
	for (; ;)
	{
		...
		for (int test_time = 0; test_time < test_times; test_time++)
		{
			...
			//查询获取信息
			tree->range_seek(tree_range_group, range);
			//若查询获得区块数量与预期不一致 
			if (tree_range_group.size() < block_num_expected ||
				tree_range_group.size() > block_num_expected)
				cout << format("查询结果异常\n");
			//若查询获得区块数量和预期一致
			else
				cout << format("查询结果正常\n");
			...
			cout << format("预计区块数量:{} \n",
				block_num_expected);
			cout << format("实际区块数量: ");
			cout << tree_range_group.size() << endl;
		}
	...
	const uint64_t& block_size, const coord_double& root, uint64_t test_times)
{
	...
	//坐标查询结果存储
	vector<tree_manager_handle<Test_Data>*> manager_handle_group{};
	//循环测试单点查找功能
	for (;;)
	{
		cout << format("当前测试半径: {}\n", radius_now);
		...
		for (int test_time = 0; test_time < test_times; test_time++)
		{
			...
			//初始化空指针
			manager_handle_group.push_back(nullptr);
			//查询获取信息
			manager.block_info_seek(manager_handle_group.back(), coord_group.back());
			//若查询失败
			if (manager_handle_group.back() == nullptr)
			{
				//格式化坐标存储
				coord_double format_coord = coord_group.back();
				//坐标格式化以获得该区块节点坐标
			...
			{
				//设置数值以待校验
				Test_Data::set_num(*(manager_handle_group.back()->ptr_data), gen(-radius_now, radius_now));
				//继续循环
				continue;
			}
		}
		...
		for (int test_time = 0; test_time < test_times; test_time++)
		{
			...
			//若两次查询结果都为空句柄
			if (examiner == nullptr || manager_handle_group[test_time] == nullptr)
			{
				//格式化坐标存储
				coord_double format_coord = coord_group[test_time];
				//坐标格式化以获得该区块节点坐标
			...
			//进行查询结果检验
				...
				Test_Data::compare(*(manager_handle_group[test_time]->ptr_data), *(examiner->ptr_data));
		}

		//重置存储器并且释放内存
		coord_group.clear();
		manager_handle_group.clear();
