...
//单点查询可行性分析
template <typename T>
int Quadtree<T>::point_seekable_analyse(const coord_int& target)
{
	/*函数逻辑：
			  0，代表分析已经结束，查找不可行
			  1，代表分析正在支持，查找可能可行
...
//范围查询可行性分析
template <typename T>
void Quadtree<T>::range_seekable_analyse(const coord_range& format_range, coord_range& seekable_range)
{
	for (;;)
	{
		//计算四叉树当前查询范围
		...
		//获取是否扩大标记
		coord_double expand_sign = seekable_range_calcu(format_range,seekable_range);
