...
namespace engine
{
	...
	void Quadtree<T>::range_seekable_analyse(const coord2D_range& format_range, coord2D_range& seekable_range)
	{
		for (;;)
		{
			...
			//获取是否扩大标记
			coord2D_double expand_sign = seekable_range_calcu(format_range, seekable_range);

			//若返回坐标非树根节点坐标
			//则进行扩大(若存在管理层则进行申请)
			if (expand_sign != state.root)
			{
				//若存在回调则进行扩大申请
				if (callback)
				{
					//若扩大申请通过通过则扩大
					if (callback(state.root, expand_sign))
					{
						//若扩大失败则直接结束计算
						if (!tree_expand())
							break;
	...
	//最小区块单元查找
	template <typename T>
	void Quadtree<T>::block_seek(tree_block_data<T>*& receiver, const coord2D_int& target, bool stable)
	{
		//四叉树上限上限临时存储
		...
		if (receiver == nullptr)
		{
			receiver = new(std::nothrow) tree_block_data<T>;
			//若内存分配失败则返回
			if (receiver == nullptr)
				return;
		}
	...
	//范围区块单元查找
	template <typename T>
	void Quadtree<T>::range_seek(std::vector<tree_block_data<T>*>& receiver, const coord2D_range& target_range, bool stable)
	{
		//可查询范围存储
			...
			else if (recur_level_now == recur_level_max - 1)
			{
				...
				for (; recur_direct <= SE; recur_direct++)
				{
					...
					if (!child_node_recur(child_node, recur_direct, LEAF, stable))
						continue;
					//记录查询结果
					auto* new_data = new(std::nothrow) tree_block_data<T>
