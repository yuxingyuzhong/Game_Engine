...
//单区块信息查询
template<typename T>
void Quadtree_Manager<T>::seek(tree_block_data<T>*& receiver,const coord_int& target, bool stable)
{
    //查询四叉树记录
...
//范围区块信息查询
template<typename T>
void Quadtree_Manager<T>::seek(vector<tree_block_data<T>*>& receiver,
    const coord_range& target_range, bool stable)
{
    //简化表示路径
