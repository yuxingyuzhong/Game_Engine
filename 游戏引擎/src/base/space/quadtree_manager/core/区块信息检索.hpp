...
namespace engine
{
    ...
    //单区块信息查询
    template<typename T>
    void Quadtree_Manager<T>::seek(tree_block_data<T>*& receiver, const coord2D_int& target, bool stable)
    {
        //查询四叉树记录
    ...
    //范围区块信息查询
    template<typename T>
    void Quadtree_Manager<T>::seek(std::vector<tree_block_data<T>*>& receiver,
        const coord2D_range& target_range, bool stable)
    {
        ...
        //四叉树返回结果存储
        std::vector<tree_block_data<T>*> buffer{};
