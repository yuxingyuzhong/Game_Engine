...
template<typename T>
void Quadtree_Manager<T>::prepare_smart_create_params(const vector<coord_int>& coord_set,
...
template<typename T>
void Quadtree_Manager<T>::divide_single_father_block(const vector<coord_int>& coord_set,
...
{
    // 辅助：判断点是否在当前区块内
    auto in_block = [&](const coord_int& p) -> bool {
        return p.X >= block_left && p.X <= block_right &&
            p.Y >= block_down && p.Y <= block_up;
        };

    // 收集当前区块内的所有点（复制子集）
    vector<coord_int> local_points;
    ...
    {
        if (fabs(p.X - center_x) > limit || fabs(p.Y - center_y) > limit)
        {
            centered = false;
            break;
        }
