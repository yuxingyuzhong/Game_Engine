...
namespace engine
{
    ...
    public:
        ...
        // ---- 设置 ----
        //数据迁移方法注册
        void callback_sign(const std::function<void(tree_block_data<T>& receiver, 
        ...
        // ---- 查询 ----
        //单区块信息查询
        void seek(tree_block_data<T>*& reciver,const coord2D_int& target, bool stable);
        //范围区块信息查询
        void seek(std::vector<tree_block_data<T>*>& receiver, const coord2D_range& target_range, bool stable);
    ...
    private:
        ...
        //外界上级管理对象回调管理方法
        std::function<void(tree_block_data<T>& receiver, tree_block_data<T>& transmiter)> copy;  //数据迁移方法
