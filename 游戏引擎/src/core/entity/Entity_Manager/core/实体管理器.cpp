...
namespace engine
{
    ...
    //初始化脚本路径注册注册
    void Entity_Manager::intialize_script_register(const string& entity_type, const string& path)
    {
        //若当前实体类型未注册初始化脚本
    ...
    void Entity_Manager::entity_build(const string& type, const vector<int64_t>& IDs)
    {
        ...
        if (initialize_scripts.count(type))
        {
            ...
            for (int build_time = 0; build_time < IDs.size(); build_time++)
            {
                ...
                (new(nothrow)Dynamic_Entity(IDs[build_time], intialize_path));
                //若实体创建成功
                if (new_entity != nullptr)
                {
                    //构造待注入依赖
                    auto event_entry = [this](vector<shared_ptr<config_event>> events, any others)->void
                        {
                            //简化表示路径
                            ...
                            //若事件数量为1则处理
                            if (events.size() == 1)
                                this->inner_event_govern(events.front(), entities);
                            //若事件数量大于1则直接返回
                            else if (events.size() > 1)
                            {
                                Log::info("Entity_Manager::单次通讯仅处理一次事件\n超量请求已驳回");
                                return;
                            }
                        };
                    //设置事件发送入口
        ...
        vector<weak_ptr<Dynamic_Entity>>& minions)
    {
        ...
        if (category == "Entity")
        {
            ...
            {
                //若目标实体类型字段无效
                if (!config_checker.field_check<string>(config, "target"))
                    return;
                ...
                if (acl_index >= 0)
                {
                    ...
                    //获取目标实体类型
                    string target_type = config["target"];
                    //检查操作是否具有合法权限
            ...
            else if (tag == "Unload")
            {
                ...
                if (!config_checker.field_check<string>(config, "target"))
                    return;
                //若待卸载实体ID集合字段无效
                if (!config_checker.field_check<vector<int64_t>>(config, "ID"))
                    return;

                //获取待卸载实体类型
                string type = config["target"];
                //获取待卸载实体ID
                vector<int64_t> IDs = config["ID"];
            ...
            {
                //若目标实体类型字段无效
                if (!config_checker.field_check<string>(actual_event->config, "target"))
                    return;
                //若目标实体ID字段无效
                ...
                for (int match_time = 0; match_time < transfer_records.size(); match_time++)
                {
                    ...
                    //若信息记录一致则进行下一步操作
                    if (original_event->sender == actual_event->config["target"].get<string>() &&
                        original_event->config["target"].get<string>() == actual_event->sender &&
    ...
    void Entity_Manager::outer_event_process(shared_ptr<config_event> evt)
    {
        ...
        if (evt->category == "Config")
        {
            ...
            {
                //记录实体类型初始化路径
                intialize_script_register(config["type"], config["initialize_path"]);
                //记录实体类型从属权限
        ...
        else
        {
            ...
            if (tag == "Build")
            {
                ...
                //获取实体创建数目
                int counts = config.value<int>("counts", 0);
                //若创建数目已指定
                if(counts != 0)
                    //创建对应实体
                    entity_build(config["target"], counts);
                else
                {
                    //获取ID编号集合
