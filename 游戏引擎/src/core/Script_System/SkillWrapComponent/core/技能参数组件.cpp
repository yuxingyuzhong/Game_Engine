#include "src/core/Script_System/SkillWrapComponent/局部命名空间使用.h"
...
namespace engine
{
    ...
    //通用属性槽注册
    void SkillWrapComponent::property_slot_sign(const unordered_map<std::string, double>& property_slot)
    {
        //接收数据
    ...
    //行为决策脚本功能注册
    void SkillWrapComponent::function_sign(LuaState& action_script)
    {
        action_script.set_function("wrap_script_sign", [this](const std::string& name, const std::string& path)->void
            {
