...
namespace Game_Engine
{
	...
	public:
		...
		//一键生成整数坐标
		coord_int gen_coord(const int64_t& min, const int64_t& max)
		{
			//坐标临时存储
			coord_int coord{};
			//生成X轴坐标
		...
		//一键生成坐标范围
		coord_range gen_range(const int64_t& min, const int64_t& max)
		{
			//坐标范围临时存储
			coord_range range{};
			//循环生成坐标范围
		...
		//坐标格式化
		void format_coord(coord_double& coord, const coord_double& root, const uint64_t block_size)
		{
			// 匹配坐标临时存储（逐步调整至最终区块中心）
			coord_double catch_coord = root;
		...
		//范围格式化
		void format_range(coord_range& range, const coord_double& root, const uint64_t block_size)
		{
